#!/usr/bin/perl -w
# very simple cutter for 4 letter restriction enzymes
...
while (<>) {				# get user input as a file or stdin
	...
	for ( $i = 0 ; $i < length($seq) - 3 ; $i++ ){			# construct words starting from the 5' end
		$testseq = substr( $seq, $i, 4 );					# get every possible 6 letter word substring

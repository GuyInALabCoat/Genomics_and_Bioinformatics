...
while (<>) {                           # user input as file or stdin
    ...
    translate($rnaseq, 0);         # amino acids, reading frame starting from 0
    translate($rnaseq, 1);         # amino acids, reading frame starting from 1 
    translate($rnaseq, 2);         # amino acids, reading frame starting from 2 
...
sub translate {
    ...
    for ( $i = "$_[1]" ; $i < length($rnaseq) - 2 ; $i = $i + 3 ) {     # translation
	$codon = substr($rnaseq, $i, 3);                          # moving three bases at a time, extract one codon at a time
